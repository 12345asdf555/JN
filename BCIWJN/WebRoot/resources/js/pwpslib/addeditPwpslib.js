...
function editWpslib() {
	...
	if (row) {
		...
		url = "wps/updatePwpslib?fid=" + row.fid;
	}
}
...
function addMainWps() {
	...
	url = "wps/addMainPwps?fid=" + wlrow.fid;
	$('#mwdlg').window('open');
	return;
	if (wlrow.model == 174) {
		EPWINIT();
...
function editMainWps(indexrow,row) {
	...
	url = "wps/updateMainPwps?fid=" + indexrow.fid;
	return;
	if (row) {
		if (row.model == 174) {
			EPWINIT();
...
//					$("#fweld_method").combobox('select', result.ary[0].value);
					$("#dfweld_method").html(boptionStr);
					$("#dfweld_method").combobox();
					$("#tfweld_method").html(boptionStr);
					$("#tfweld_method").combobox();
				}
			}
		},
		error : function(errorMsg) {
		...
		success : function(result) {
			if (result) {
				if (result.ary.length != 0) {
					...
					$("#fpower_polarity").html(boptionStr);
...
//					$("#fweld_position").combobox('select', result.ary[0].value);
					...
					$("#tfweld_position").html(boptionStr);
					$("#tfweld_position").combobox();
				}
			}
		},
		error : function(errorMsg) {
		...
		success : function(result) {
			if (result) {
				if (result.ary.length != 0) {
					...
					$("#tfbase_material_id").html(boptionStr);
					$("#tfbase_material_id").combobox();
				}
			}
		},
		error : function(errorMsg) {
		...
		success : function(result) {
			if (result) {
				if (result.ary.length != 0) {
					...
					$("#tfweld_material_id").html(boptionStr);
					$("#tfweld_material_id").combobox();
				}
			}
		},
		error : function(errorMsg) {
...
function openTaskDialog(){
	...
	$('#swldlg').window('center');
	$('#swldlg').window('open');
	$("#tweldedJunctionno").textbox('setValue',row.wpslibName);
	$('#tfweld_method').combobox('setValue', row.fweld_method);
	$('#tfweld_position').combobox('setValue', row.fweld_position);
	$('#tfwpslib_id').combobox('setValue', row.fid);
	$.ajax({
		type : "post",
		async : false,
		url : "wps/getBaseMaterialById?fid="+row.fbase_material_id_one,
		data : {},
		dataType : "json", //返回数据形式为json  
		success : function(result) {
			if (result) {
				$('#tfbase_material_id').combobox('setValue', result.rows.ftype);
			}
		},
		error : function(errorMsg) {
			alert("数据请求失败，请联系系统管理员!");
	...
	$.ajax({
		type : "post",
		async : false,
		url : "wps/getWeldMaterialById?fid="+row.fweld_material_id,
		data : {},
		dataType : "json", //返回数据形式为json  
		success : function(result) {
			if (result) {
				$('#tfweld_material_id').combobox('setValue', result.rows.ftype);
			}
		},
		error : function(errorMsg) {
			alert("数据请求失败，请联系系统管理员!");
...
function generateTask(){
	var wtno = $("#tweldedJunctionno").textbox('getValue');
	var startTime = $("#tstartTime").datetimebox('getValue');
	var operatorId = $('#tfwelder_id').combobox('getValue');
	var wps_lib_id = $('#tfwpslib_id').combobox('getValue');
	var fweld_position = $('#tfweld_position').combobox('getValue');
	var fweld_method = $('#tfweld_method').combobox('getValue');
	var fweld_material_id = $('#tfweld_material_id').combobox('getValue');
	var fbase_material_id = $('#tfbase_material_id').combobox('getValue');
	var flag=0;
	var messager = "";
	var url2 = "";
	messager = "生成成功！";
